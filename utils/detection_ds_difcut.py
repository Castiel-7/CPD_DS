...
class DetectionDS(object):
    def __init__(self, window_length=128, order=64, lag = 64, M = 5, N = 10, DS_dim = 5, PS_dim = 30, dif_cut = 0):
    ...
    def fit(self, x):
        ...
        for t in range(start_idx, end_idx):
            #print("train", self.lag, self.PS_dim, t)
    ...
    def predict(self, x):
        ...
        #print(end_idx)
        for t in range(start_idx, end_idx):
            #print("test", self.lag, self.PS_dim, t)
            for i in range(x.shape[0]):
                train_H = self._get_hankel(x[i, :], order = self.order,
                                        start = t,
                                        end = t + self.window_length)
            ...
            if similarity_mean5 < 0:
                similarity_mean5 = similarity_mean5 * (-1)
            ...
            if similarity_mean_all < 0:
                similarity_mean_all = similarity_mean_all * (-1)
